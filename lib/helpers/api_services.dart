...
Future getPlayerStat({gameId, timeStart, timeEnd, action, teamId}) async {
  String token = await getToken();
  ...
  try {
    http.Response response = await http.get(
      Uri.parse(baseUrl +
          "api/v1/action_count_for_individual?gameId=$gameId&timeStart=$timeStart&timeEnd=$timeEnd&action=$action$team"),
      headers: headers,
    );
...
Future getTeamsStat({gameId, timeStart, timeEnd, action,zone}) async {
  ...
  String zonevalue = zone == null ? "" : "&action=$zone";
